...
async def show_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for product in products:
        caption = (
            ...
            f"💰 {product['price']:.2f}\n"
            f"📦 {t('label_stock', lang)}: {product['stock_quantity']}"
        )
...
async def finalize_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for product_id, qty in cart.items():
        db.decrease_stock(product_id, qty)
...
async def handle_product_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin uploads a .txt or .csv file, one product per line, pipe-delimited:
    Name | Description | Price | ImageURL | Category | Stock
    Blank lines and lines starting with # are skipped.
    """
    if not is_admin(update):
        return  # ignore silently for non-admins uploading random files

    document = update.message.document
    if not document.file_name.lower().endswith((".csv", ".txt")):
        await update.message.reply_text("Please upload a .csv or .txt file (one product per line).")
        return
    ...
    try:
        ...
        await update.message.reply_text("Couldn't read that file — please save it as UTF-8 text.")
        return
    ...
    for line_num, raw_line in enumerate(text.splitlines(), start=1):
        ...
        if len(parts) != 6:
            errors.append(f"Line {line_num}: expected 6 fields (Name|Description|Price|ImageURL|Category|Stock), got {len(parts)}")
            continue

        name, description, price, image_url, category, stock = parts
        try:
            price = float(price)
            stock = int(stock)
        except ValueError:
            errors.append(f"Line {line_num}: price/stock must be numbers")
            continue

        db.add_product(name, description, price, image_url or None, category, stock)
        added += 1

    reply = f"✅ Added {added} product(s)."
    if errors:
        reply += "\n\n⚠️ Issues:\n" + "\n".join(errors[:10])
...
async def add_product_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Usage: /addproduct Name | Description | Price | ImageURL | Category | Stock"""
    if not is_admin(update):
    ...
    if len(parts) != 6:
        await update.message.reply_text(
            "Usage:\n/addproduct Name | Description | Price | ImageURL | Category | Stock\n\n"
    ...
    try:
        ...
        await update.message.reply_text("Price must be a number and Stock must be a whole number.")
        return

    db.add_product(name, description, price, image_url or None, category, stock)
    await update.message.reply_text(f"Added '{name}' to category '{category}' with {stock} in stock.")
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Document.ALL, handle_product_file))
